(ns gsacompliance.operation
  "OperationActor -- one GSA compliance operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (GSACompliance-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the GSA Federal-Procurement
  ...
  One graph run = one compliance operation (sam-registration track or
  mas-schedule track). No unbounded inner loop.
  ...
  governor is clean -- see `gsacompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [gsacompliance.gsacompliancellm :as gsacompliancellm]
            [gsacompliance.governor :as governor]
            [gsacompliance.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (gsacompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (gsacompliancellm/-advise advisor store request)]
            {:proposal p :audit [(gsacompliancellm/trace request p)]})))
